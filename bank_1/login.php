...
if (isset($_GET['logout'])) {
    ...
    header('Location: http://localhost/nd/bank_1/login/login.php');
    die;
}
    ...
    foreach ($users as $user) {
        if ($_POST['name'] === $user['name']) {
            if (md5($_POST['psw']) === $user['psw']) {
                $_SESSION['logged'] = 1;
                $_SESSION['message'] = 'Welcome to DE FOREST, Dear';
                $_SESSION['msg_type'] = 'ok';
                $_SESSION['name'] = $user['name'];
                header('Location: http://localhost/nd/bank_1/login/authorized.php');
                die;
            }
        }
    }
    ...
    <main class="login">
        <form action="./login.php" method="post">
            <div class="mb-3">
                ...
                <div id="emailHelp" class="form-text">Enter you user name HERE.</div>
            </div>
            <div class="mb-3">
